...
The images below are **UI design mockups**, programmatically drawn by `scripts/generate_readme_screenshots.mjs` (SVG → PNG) to illustrate each feature module's layout and interaction — **they are not live screenshots of the running app**. We follow a xianxia serial author's journey to walk through idea capture, manuscript management, chapter writing, knowledge management, structure tracking, analytics, and model configuration. The mockups use offline demo data and never read real API keys. Real screenshots require a Windows desktop / Android device UAT and will replace these once available.
...
This README uses 21 **UI design mockups** stored in `docs/readme/screenshots/` as product showcase material. They are programmatically drawn by `scripts/generate_readme_screenshots.mjs` (SVG → PNG) with offline demo data and no real API keys; **they are design mockups, not live screenshots of the app**. Real screenshots require a Windows desktop / Android device UAT and will replace these once available.

## v1.3 User Journey Showcase

v1.3 centers on a "100-chapter xianxia novel user-journey validation," turning verification results scattered across test code and logs into directly readable evidence. Below is a visual preview and key conclusions; the full interactive version (dark theme, chapter-toc navigation, live data) is available by cloning the repo and opening [`index.html`](docs/v1.3-user-journey/index.html) in a browser.

### Showcase Home

![v1.3 showcase home](docs/v1.3-user-journey/screenshots/index.png)

**Key metrics**　Chapters **100**　·　Validation phases **12–16**　·　Automated tests **1169+**　·　Status **Passed**

### Validation Flow

Stepping into the "big story, rough prose" user perspective, MuseFlow completes the full creation loop from world-building to 100-chapter export:

```mermaid
flowchart LR
    A["World-building<br/>xianxia template · character cards · Skill guards"] --> B["Capture & organize<br/>PromptPipeline"]
    B --> C["Opening guidance<br/>scene / character / suspense"]
    C --> D["100-chapter flow<br/>300–500 word bound"]
    D --> E["Structure & export<br/>foreshadowing · MD/TXT/JSON"]
    E --> F["Analysis reports<br/>tokens · pain points · anti-AI-scent · consistency"]
```

### Validation Results

![v1.3 validation report](docs/v1.3-user-journey/screenshots/validation-report.png)

- ✅ `flutter analyze` — no issues
- ✅ `flutter test` — full suite passed (`All tests passed!`)
- ✅ Deterministic user-journey test completed the 100-chapter flow
- ⚠️ Real external-model tests skipped as expected when `GLM_API_KEY` / `OPENAI_API_KEY` are missing

### 《剑道苍穹》100-Chapter Sample Preview

![100-chapter xianxia sample](docs/v1.3-user-journey/screenshots/xianxia-sample.png)

The sample traces a mortal's path from initiation → foundation → golden core → nascent soul → ascension across 100 deterministic chapters, validating loop closure and output bounds. It is **a reference and acceptance artifact, not a final literary manuscript**.

### Full Interactive Version & Evidence

- Interactive pages: [home](docs/v1.3-user-journey/index.html) · [100-chapter sample](docs/v1.3-user-journey/xianxia-100-chapter-sample.html) · [validation report](docs/v1.3-user-journey/validation-report.html)
- [Chapter JSON data](docs/v1.3-user-journey/data/chapters.json) (source: `test/journey/helpers/story_outline.dart`)
- Test evidence: [`full_journey_test.dart`](test/journey/full_journey_test.dart) · [`automated_ui_evidence_test.dart`](test/journey/automated_ui_evidence_test.dart)
