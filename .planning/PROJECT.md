...
- [ ] 作者风格指纹分析与动态Prompt适配
- [ ] Few-shot作者样本注入系统
- [ ] 反AI味词库扩展（15→200+）与语义级检测
- [ ] 文风温度计仪表盘（AI味指数、风格一致性、文学性评分）
- [ ] 知识库模糊匹配与别名扩展
- [ ] 实体关系图谱与上下文感知注入
- [ ] 自动伏笔提醒（写作时实时提示关联伏笔）
- [ ] 章节上下文链增强（前N章摘要+故事弧位置）
- [ ] 多轮AI对话（连续对话保持上下文）
- [ ] 智能续写引导（方向性建议，保持作者决策）
- [ ] 编辑器AI操作增强（扩写/缩写/对话生成/环境描写）
- [ ] AI操作撤销历史（20步历史栈）
- [ ] Web端响应式优化与写作进度仪表盘
...
**Shipped:** v1.0 MVP + v1.1 创作体验升级 + v1.2 多文稿架构 + v1.3 用户旅程分析报告。
**Codebase:** Flutter/Dart app with capture→synthesis→editor pipeline, knowledge base, skill system, story structure, template library, analytics, story arc visualization, multi-manuscript chapter editor, and a reports center for token cost, pain points, blind-read evaluation, and knowledge-base consistency analysis.
...
**Coverage:** Automated `flutter analyze` and `flutter test` pass after Phase 16. Human UAT remains for physical device/IME/lifecycle validation and the Phase 16 report UX/semantic-quality checks.
