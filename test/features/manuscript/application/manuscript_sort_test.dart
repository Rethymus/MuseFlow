...
void main() {
  ...
  group('compareManuscripts', () {
    test('recentEdit sorts by updatedAt descending', () {
      ...
      expect(result, greaterThan(0)); // older > newer means older sorts after newer

      final reverse = compareManuscripts(newer, older, ManuscriptSortMode.recentEdit);
      expect(reverse, lessThan(0)); // newer < older means newer sorts before older
    });
