...
void main() {
  Chapter chapter({String content = '林风入门青云宗，初识苏雪晴，被安排到杂役房。'}) => Chapter(
    id: 'ch-1',
    manuscriptId: 'ms-1',
    title: '第一章',
    sortOrder: 1,
  ...
  ChapterSummaryRefreshService buildService({
    ...
    return ChapterSummaryRefreshService(
      ...
      summaryRepository: repository ?? _FakeSummaryRepository(),
    );
  }

  group('ChapterSummaryRefreshService.refreshIfNeeded', () {
    test('T1: no stored summary + >=20 chars -> summarize once + put once + '
        'RefreshOutcome(refreshed: true, summary non-null)', () async {
      ...
      expect(outcome.summary!.summary, '林风入门青云宗，与苏雪晴初识。');
    ...
    test('T4: force refresh() on a fresh stored summary -> adapter called 1x, '
        'put 1x, RefreshOutcome(refreshed: true)', () async {
      ...
      final outcome = await service.refresh(c, now: DateTime(2026, 6, 3));
    ...
    test(
      ...
      () async {
        ...
        expect(
          c.wordCount,
          lessThan(ChapterSummaryRefreshService.minSummaryChars),
        );
    ...
    test('T6 (reliability): summarize throws AIStreamException -> service '
        'RE-THROWS; no partial put occurred', () async {
    ...
    test(
      ...
      () async {
        ...
        final refreshedCount = outcomes.where((o) => o.refreshed).length;
  ...
  group('ChapterSummaryRefreshService.deleteSummary', () {
    test('T7: deleteSummary(chapterId) removes the stored row -> '
        'getByChapterId returns null after', () async {
    ...
    test('T8 (reliability): repository.delete throws StateError -> service '
        'RE-THROWS (same surfacing posture as put/summarize)', () async {
      final adapter = _FakeAdapter(['unused']);
      final repository = _FakeSummaryRepository();
...
class _FakeAdapter extends OpenAIAdapter {
  _FakeAdapter(this.chunks) : error = null, callCount = 0;
  _FakeAdapter.withError(this.error) : chunks = const [], callCount = 0;
