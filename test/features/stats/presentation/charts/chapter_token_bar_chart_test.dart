  ...
  group('ChapterTokenBarChart', () {
    testWidgets('renders empty state text when no records provided', (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: ChapterTokenBarChart(records: []),
          ),
        ),
      );
    ...
    testWidgets('renders empty state when no records have chapterId', (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: ChapterTokenBarChart(records: records),
          ),
        ),
      );
    ...
    testWidgets('aggregates totalTokens per chapterId and renders one bar per chapter', (tester) async {
      final now = DateTime.now();
      final records = [
        TokenAuditRecord(
          id: '1',
