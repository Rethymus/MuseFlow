...
Editor _createTestEditor([String text = 'Hello world']) {
  final document = MutableDocument(
    nodes: [ParagraphNode(id: 'test-node', text: AttributedText(text))],
  );
...
void main() {
  group('Editor formatting', () {
    ...
    group('bold formatting', () {
      test(
        'should apply boldAttribution to selected text via ToggleTextAttributionsRequest',
        () {
          ...
          expect(
            spans,
            isNotEmpty,
            reason: 'Bold attribution should be applied to selected text',
          );
        },
      );

      test(
        'should toggle composer preferences for bold when selection is collapsed',
        () {
    ...
    group('italic formatting', () {
      test(
        'should apply italicsAttribution to selected text via ToggleTextAttributionsRequest',
        () {
          // Select characters 0-5 ("Hello")
          final selection = DocumentSelection(
            base: DocumentPosition(
          ...
          expect(
            spans,
            isNotEmpty,
            reason: 'Italic attribution should be applied to selected text',
          );
        },
      );
    });
    ...
    group('heading conversion', () {
      test(
        'should convert paragraph to H1 with header1Attribution blockType',
        () {
          final node = _firstNodeAs<ParagraphNode>(editor.document);
      ...
      test(
        'should convert paragraph to H2 with header2Attribution blockType',
        () {
          final node = _firstNodeAs<ParagraphNode>(editor.document);
      ...
      test(
        'should convert paragraph to H3 with header3Attribution blockType',
        () {
          final node = _firstNodeAs<ParagraphNode>(editor.document);
    ...
    group('toolbar active state', () {
      test('should detect bold in composer preferences when set', () {
        ...
        final isBoldActive = editor.composer.preferences.currentAttributions
            .contains(boldAttribution);
        expect(
          isBoldActive,
          isTrue,
          reason: 'Bold should be detected as active in preferences',
        );
      });
      ...
      test('should not show bold as active when not set', () {
        ...
        expect(
          isBoldActive,
          isFalse,
          reason: 'Bold should not be active when not toggled',
        );
      });
      ...
      test('should detect bold attribution on text at expanded selection', () {
        ...
        expect(
          spans,
          isNotEmpty,
          reason: 'Text should have bold attribution applied',
        );
      });
    });
  });
