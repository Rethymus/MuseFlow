...
void main() {
    ...
    test('should have 100 chapter headers in sequential order', () {
      ...
      for (final chapter in chapterExports) {
        expect(
          ...
          reason: 'Missing Markdown title ${chapter.title}',
        );
      }
      debugPrint(
        '[D-09] markdown headers=${headerMatches.length}, length=${markdown.length}',
      );
    });
    ...
    test('should include non-empty content after each chapter title', () {
      ...
      for (final chapter in chapterExports) {
        ...
        expect(
          ...
          reason: 'Missing content after ${chapter.title}',
        );
      }
    });
  });
  ...
  group('JSON format validation', () {
    test('should produce parseable JSON with complete metadata', () {
      ...
      debugPrint(
        '[D-09] json chapters=${chapters.length}, length=${json.length}',
      );
    });
  });
  ...
  group('Cross-format export validation', () {
    test('should preserve chapter content across Markdown, TXT, and JSON', () {
      ...
      final txt = exportService.buildTxt(bundle);
      final decoded =
          jsonDecode(exportService.buildJson(bundle)) as Map<String, dynamic>;
      final jsonChapters = decoded['chapters'] as List<dynamic>;

      for (var i = 0; i < chapterExports.length; i++) {
        final chapter = chapterExports[i];
        final contentSample = chapter.content.substring(
          0,
          min(80, chapter.content.length),
        );
        expect(
          markdown,
          contains(contentSample),
          reason: 'Markdown missing content for ${chapter.title}',
        );
        expect(
          txt,
          contains(contentSample),
          reason: 'TXT missing content for ${chapter.title}',
        );
        expect(
          (jsonChapters[i] as Map<String, dynamic>)['content'],
          chapter.content,
        );
      }
    });
    ...
    test('should keep file sizes in expected range per D-09', () {
      ...
      debugPrint(
        '[D-09] sizes md=${markdown.length}, txt=${txt.length}, json=${json.length}',
      );
    });
  });
}
