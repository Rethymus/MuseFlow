...
| Latest audited release commit | `ec169cf chore(release): bump version to v0.1.4` |
...
Remote CI and release observation used authenticated `gh`/GitHub API evidence plus Kimi WebBridge browser pages for Actions and Release visibility. The final audit must verify the current `main` run at the time of completion rather than relying on a stale hard-coded run id.
...
| `dart format --set-exit-if-changed .` | FAIL initially, then PASS after formatting | Initial run formatted 219 files; v0.1.4 release rerun formatted 401 files with 0 changes |
...
| `flutter test` | PASS | 1179 tests passed, 12 skipped |
...
| P0 | GitHub CI/CD absent | Fixed and remotely verified | Added CI and release workflows; `main` CI run `27211536093` passed for commit `ec169cf96e22147951578045709ba12966b314af` |
| P0 | Platform artifact builds unverified | Fixed and release-verified | Android, Linux, and Web builds pass locally or in CI; release workflow published Android, Linux, Windows, and Web artifacts |
...
| P0 | GitHub Release publication for v1.4 hardening | Fixed and verified for `v0.1.4` | Published `v0.1.4`; Android/Linux/Windows/Web artifacts and `SHA256SUMS.txt` are present; checksum verification passed |
