...
class SuperKeyboardAndroid {
  ...
  Future<void> enablePlatformLogging(
      {bool sendPlatformLogsToDart = false}) async {
    await _methodChannel.invokeMethod(
        "startLogging", {"sendPlatformLogsToDart": sendPlatformLogsToDart});
  }
  ...
  Future<void> _onPlatformMessage(MethodCall message) async {
    switch (message.method) {
      ...
      default:
        SKLog.android
            .warning("Unknown Android plugin platform message: $message");
    }
  }
  ...
  void _printAndroidLog(MethodCall channelMessage) {
    ...
    final logMessage =
        "SK Android: ${channelMessage.arguments["message"] ?? "EMPTY MESSAGE"}";
    switch (level) {
      case "v":
