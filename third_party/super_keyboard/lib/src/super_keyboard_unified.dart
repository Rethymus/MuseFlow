...
class SuperKeyboard {
  ...
  Future<void> enablePlatformLogging(bool isEnabled) async {
    if (defaultTargetPlatform == TargetPlatform.iOS) {
      SKLog.unified.fine(
          "SuperKeyboard - ${isEnabled ? "Enabling" : "Disabling"} logs for iOS.");
      if (isEnabled) {
        await SuperKeyboardIOS.instance
            .enablePlatformLogging(sendPlatformLogsToDart: true);
      } else {
      ...
      if (isEnabled) {
        await SuperKeyboardAndroid.instance
            .enablePlatformLogging(sendPlatformLogsToDart: true);
      } else {
