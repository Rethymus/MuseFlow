...
- **Global scope**: `/home/re/code/MuseFlow/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/re/code/MuseFlow/.claude/notes/`)
...
2. Glob `/home/re/code/MuseFlow/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/re/code/MuseFlow/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/re/code/MuseFlow/.claude/notes/` — works in any directory
