...
/// Runs a scroll benchmark on a super_editor document.
///
...
class SuperEditorBenchmarkRunner {
  SuperEditorBenchmarkRunner({
  ...
  void loadText(String text) {
    final document = editor.context.find<MutableDocument>(
      Editor.documentKey,
    );
    ...
    for (var i = 0; i < paragraphs.length; i++) {
      nodes.add(
        ParagraphNode(
          id: 'para-$i',
          text: AttributedText(paragraphs[i]),
        ),
      );
    }

    // Clear existing nodes by ID, then add new ones
    final existingNodes = document.toList();
