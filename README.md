...
本 README 的产品展示素材中，**文稿库**（01）、**写作统计**（15）、**Token 审计**（16）、**分析报告**（17）、**报告详情**（18）、**设置**（19）、**AI 模型管理**（20）与 **AI 用语过滤**（21）为真实 widget 渲染（由 golden 测试生成），其余为 `scripts/generate_readme_screenshots.mjs` 程序化绘制的设计示意图。所有图片使用离线示例数据，不读取真实密钥；真实截图正由 golden 测试逐页迁移。
