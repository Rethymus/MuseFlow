...
<p align="center">
  <img src="docs/readme/logo/museflow-logo.png" alt="MuseFlow 灵韵 logo：由章节、伏笔和故事弧组成的结构星图" width="760">
</p>
...
2026 年 8 月 18 日，起点中文网做了一件让整个网文圈炸锅的事：凡是被系统判定为使用过 AI 的小说，月票榜与畅销榜上的排名一律撤销。被处理与否一眼可辨——正常小说哪怕只有个位数的月票，也会显示“月票榜第 1~2 万名”；而某些拥有几百上千月票的小说，点开一看，排名一栏却只剩一条横杠。据社区不完全统计，仅明确被处理的万订小说就有 25 本，其中不乏三五万均订、被视为“十二天王”预备役的头部作品；连 6 位大神作者与 1 位十二天王作者的作品，也未能在这场清剿中幸免。

被处理的头部作者里，只有一位正面回应。他解释说，自己只是用 AI 修改错字语病、整理段落；可细看他举的例子，字词的顺序与表达方式早已被整体改写——与其说是纠错，不如说已算得上润色。若整部书都是这样改出来的，被判定为 AI 好像也不算冤。况且平台判定依据的是整部作品的 AI 占比，而非只言片语，网传超过一成才会动刀。尺度并不严苛，只会有漏杀、很难有误杀；事后那些“只是改改语病”的解释，听一听就好。普通作者用 AI 也就用了；身居大神之列，却仍不爱惜羽毛、不珍视读者，才是真正让人唏嘘的地方。好在平台这波一视同仁，白金作者也无一在列。

而在读者这一侧，下榜的实际影响远比表面沉重。撤榜只是第一步，它本身就是一种变相的官方标注：这本书，用过 AI。不知情时愿意订阅追读的读者，一旦知情便多半转身离开；作品仍在正常更新，但留下来的，大概只剩不在意它是不是 AI 文的读者，与纯粹的忠实书友。月票榜与畅销榜对引流的意义不必多说——下榜之后，这些作品的新增收藏由日增千余，一路转为肉眼可见的净流失。也有知名作者把话说得糙、理却不糙：读者掏的钱，买的从来不是付费 AI 文——就像打着“现炒”名头的预制菜，再好吃，那也是预制菜。

正是这些声音，让我们重新审视 MuseFlow 的初衷：让想象力丰盈却拙于表达的人，也能借 AI 之翼把脑海中的世界落到纸面；让已在连载路上的作者，写得更快，也写得更好。可这次事件展示了另一面——哪怕只是稍加润色，哪怕出自热门、资深作者之手，处理也无一例外。评论区的态度则更加直白：AI 写出的文字再甜腻，也不过是一具装不进情感的空皮囊，终究读不懂角色的悲欢。这样看来，读者按下订阅键的那一刻，买下的从来不只是情节，还有字里行间那一点人的温度——难道真的是“付费读者也要情绪价值”？

仅凭一次事件，我们不下定论，也不急于站队。这一次，MuseFlow 选择以中立的视角、不偏不倚地研究这个问题的本质：AI 在创作中究竟应当扮演什么角色，边界在哪里，又能为作者与读者各自带来什么。相信不久之后，MuseFlow 会给出属于自己的新解释，敬请期待。

> 以上事件脉络与观点转引自小黑盒作者“雾野漫游”的文章《起点斩杀AI文，竟然有6位大神作者和1位12天王作者的小说也被处理》([原文链接](https://www.xiaoheihe.cn/app/bbs/link/188604679))与《知名恋爱文作者蜜汁姬用“青楼比喻”评价AI文，真的是话糙理不糙》([原文链接](https://www.xiaoheihe.cn/app/bbs/link/09687254ceb9))。
...
- **轻量跨平台**：Flutter 实现，本地优先存储；Android、Linux、Windows 是发布目标，Web 提供 GitHub Pages 浏览器工作区预览。浏览器中的作品采用尽力持久化，仍需定期导出备份。
...
- Android / Linux 本地构建验证，Windows GitHub Actions 构建目标，Web GitHub Pages 预览

## AI Provider BYOK

MuseFlow 采用 **BYOK（Bring Your Own Key）** 模式：用户使用自己从 OpenAI、Claude、DeepSeek、GLM 或其他兼容服务商获取的 API Key。原生客户端将密钥保存在操作系统安全存储中；Web 版仅在当前标签页会话中保存，关闭标签页后自动失效。AI 请求由 MuseFlow 客户端直接发送至用户选择的服务商，模型额度、限流与费用由对应服务商账户管理。原生客户端的本地 Ollama 模型无需 API Key；Web 端仅支持 HTTPS 且允许浏览器 CORS 的服务商。

Web 浏览器工作区使用 IndexedDB 自动保存作品，但浏览器清理站点数据或回收空间仍可能导致数据丢失。设置页提供持久化权限申请、全部作品备份和恢复；临时体验可通过 `?workspace=temporary` 启用，刷新或关闭后清除全部会话数据。
