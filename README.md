...
下面的图片是**界面设计示意图**，由 `scripts/generate_readme_screenshots.mjs` 程序化绘制（SVG → PNG），用于直观展示各功能模块的布局与交互形态，**并非应用运行时的真实截图**。我们以一名修仙长篇作者的创作旅程为线索，逐一示意灵感捕捉、文稿管理、章节写作、知识库维护、结构守护、数据分析和模型配置。示意图使用离线示例数据，不读取任何真实密钥。真实截图需 Windows 桌面 / Android 设备 UAT，待补齐后替换。
...
本 README 使用 `docs/readme/screenshots/` 下的 21 张**界面设计示意图**作为产品展示素材。图片由 `scripts/generate_readme_screenshots.mjs` 程序化绘制（SVG → PNG），使用离线示例数据，不读取真实密钥；**为设计示意，非应用运行时真实截图**。真实截图需 Windows 桌面 / Android 设备 UAT，待补齐后替换。
